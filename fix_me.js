...
function createTreeView(menu, currentNode) {
  ...
  for(let group of menu){ //메뉴에서 카테고리 꺼내오기(음료, 음식, 굿즈, 카드)
    const li = document.createElement("li");
    if(!group.children){//자식요소가 더이상 없다면
      ...
      li.textContent = group.name; //li태그에 카테고리이름 넣기
      currentNode.append(li); //나이트로 콜드브루가 콜드브루에 붙는다. 이 때 currentNode는 ul(콜드브루)
    }else{
      //자식노드가 있는 경우
    ...
    span.textContent = group.name; //카테고리이름을 span태그로 감싸고
    li.append(input, span, ul); //input, span, ul를 차례로 li밑에 붙이기
    currentNode.append(li); //root에 li 붙이기
  ...
  createTreeView(group.children,ul);
  //두번째부터는 menu배열안의 객체가 아닌, menu안의 group.childeren, 즉 자식객체에서 돌아야 한다.
  //두번째부터는 root가 아닌 카테고리안에 생성된 ul로 가야한다.
    }
  }
}
